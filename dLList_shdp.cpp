...
//
std::shared_ptr<DNode> DLList::addNode(int val)
{
  // allocates memory size of DNode, then constructs object
  // on failure throws bad alloc
  std::shared_ptr<DNode> newNode(new DNode(++_lastId, val));
...
//
void DLList::removeNode(std::shared_ptr<DNode> node)
{
...
//
std::shared_ptr<DNode> DLList::findNode(int id)
{
  bool found = false;
  std::shared_ptr<DNode> curr = _headp;
